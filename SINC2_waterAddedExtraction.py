...
for group in ['e', 's', 'r']:
    ...
    ax = sns.scatterplot(data=subset, x='sample_total_weight', 
                 y='DNA_yield', hue='water_add(ul)', palette='viridis',
                 s=80)
...
group1 = df[df['water_add(ul)'] == 200]['DNA_yield'].dropna()
group2 = df[df['water_add(ul)'] != 200]['DNA_yield'].dropna()
...
sns.boxplot(x='water_add(ul)', y='DNA_yield', data=df)
plt.title('DNA yield by water addition group')
plt.show()
